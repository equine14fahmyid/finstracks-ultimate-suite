...
const Inventory = () => {
  ...
  const movementColumns = [{
    key: 'created_at',
    title: 'Tanggal',
    render: (value: any, movement: any) => movement?.created_at ? new Date(movement.created_at).toLocaleDateString('id-ID') : '-'
  }, {
    key: 'product',
    title: 'Produk',
    render: (value: any, movement: any) => {
      if (!movement) return <div>Data tidak tersedia</div>;
      const productName = movement?.product_variant?.products?.nama_produk || movement?.product_variant?.product?.nama_produk || 'Produk tidak diketahui';
      return <div>
            <div className="font-medium">{productName}</div>
            <div className="text-sm text-muted-foreground">
    ...
    render: (value: any, movement: any) => {
      ...
      return <Badge variant={movement.movement_type === 'in' ? 'default' : 'secondary'}>
    ...
    render: (value: any, movement: any) => {
      ...
      return <span className={`font-medium ${movement.movement_type === 'in' ? 'text-green-600' : 'text-red-600'}`}>
  ...
  return <div className="container mx-auto p-6 space-y-6">
      ...
      {/* Debug Panel */}
      {showDebug && <Card className="bg-yellow-50 border-yellow-200">
          <CardHeader>
      ...
      <Card>
                ...
                <form onSubmit={handleVariantSubmit}>
                  ...
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
                      ...
                      <Select value={variantFormData.product_id} onValueChange={value => setVariantFormData(prev => ({
                      ...prev,
                      product_id: value
                    }))}>
                        <SelectTrigger>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="grid gap-2">
                        ...
                        <Input id="warna" value={variantFormData.warna} onChange={e => setVariantFormData(prev => ({
                        ...prev,
                        warna: e.target.value
                      }))} placeholder="Merah, Biru, dll" required />
                      </div>

                      <div className="grid gap-2">
                        <Label htmlFor="size">Size *</Label>
                        <Input id="size" value={variantFormData.size} onChange={e => setVariantFormData(prev => ({
                        ...prev,
                        size: e.target.value
                      }))} placeholder="S, M, L, XL, dll" required />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="grid gap-2">
                        ...
                        <Input id="stok" type="number" value={variantFormData.stok} onChange={e => setVariantFormData(prev => ({
                        ...prev,
                      ...
                      <div className="grid gap-2">
                        ...
                        <Input id="sku" value={variantFormData.sku} onChange={e => setVariantFormData(prev => ({
                        ...prev,
                        sku: e.target.value
        ...
        <CardContent>
          <DataTable columns={stockColumns} data={stock || []} loading={loading} searchable={true} searchPlaceholder="Cari produk..." />
        </CardContent>
      </Card>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          {selectedVariant && <form onSubmit={handleStockAdjustment} className="space-y-4">
              <div>
                <Label>Produk</Label>
              ...
              if (value >= 0) {
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {adjustmentData.quantity > (selectedVariant?.stok || 0) ? `Akan menambah ${adjustmentData.quantity - (selectedVariant?.stok || 0)} unit` : adjustmentData.quantity < (selectedVariant?.stok || 0) ? `Akan mengurangi ${(selectedVariant?.stok || 0) - adjustmentData.quantity} unit` : 'Tidak ada perubahan'}
                </div>
              </div>
              
              <div>
                <Label htmlFor="notes">Catatan</Label>
                <Input id="notes" value={adjustmentData.notes} onChange={e => setAdjustmentData(prev => ({
              ...prev,
