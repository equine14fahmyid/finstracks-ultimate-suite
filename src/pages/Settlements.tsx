...
const Settlements = () => {
  ...
  const columns = [{
    ...
    render: (value: any, record: any) => {
      try {
        return format(new Date(record.tanggal), 'dd MMM yyyy', {
          locale: id
        });
      } catch {
  ...
  return <div className="p-6 space-y-6">
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input id="tanggal" type="date" value={formData.tanggal} onChange={e => handleInputChange('tanggal', e.target.value)} required />
              </div>
              ...
              <div>
                ...
                <Input id="jumlah_dicairkan" type="number" value={formData.jumlah_dicairkan} onChange={e => handleInputChange('jumlah_dicairkan', parseInt(e.target.value) || 0)} placeholder="0" min="0" required />
              </div>
              ...
              <div>
                ...
                <Input id="biaya_admin" type="number" value={formData.biaya_admin} onChange={e => handleInputChange('biaya_admin', parseInt(e.target.value) || 0)} placeholder="0" min="0" />
              </div>
              
              <div>
                <Label htmlFor="keterangan">Keterangan</Label>
                <Textarea id="keterangan" value={formData.keterangan} onChange={e => handleInputChange('keterangan', e.target.value)} placeholder="Keterangan pencairan (opsional)" rows={3} />
              </div>
      ...
      <Card>
        ...
        <CardContent>
          <DataTable data={settlements} columns={columns} loading={loading} searchable={true} searchPlaceholder="Cari toko..." />
        </CardContent>
      </Card>
    </div>;
};
